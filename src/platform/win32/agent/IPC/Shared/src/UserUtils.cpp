...
void GetCurrentUsers (StringVector& loggedInUsers)
{
    ...
    DWORD result = ::NetWkstaUserEnum(NULL, 1, (LPBYTE*) &pUserInfo, 
    ...
    for (DWORD i = 0; i < entriesRead; i++){
        ...
        {
            _stprintf (userName, _T("%s\\%s"), pUserInfo[i].wkui1_logon_domain, pUserInfo[i].wkui1_username);
...
void UserUtils::GetLoggedInUsers (StringVector& users)
{
  ...
  if (::RegOpenKeyEx (HKEY_LOCAL_MACHINE, PROFILE_LIST_KEY, 0, KEY_READ, 
	...
	_tcsncpy(retKeyStr, achKey, retKeyStrLen);
	retKeyStr[retKeyStrLen]='\0';
