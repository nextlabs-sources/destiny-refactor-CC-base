...
* 
*/
void IPCStub::Run(int eventBucketNumber)
{
    m_bStopped = false;
    while (!m_bStopped)
    {
        ...
        else if (event >= WAIT_ABANDONED_0 && event < WAIT_ABANDONED_0 + numEvents)
        {
            ...
            event - WAIT_OBJECT_0 < numEvents && 
            pHandleArray[event - WAIT_OBJECT_0] == m_hReceiveEvent)
        {
            PerformHandshake();
        }
        else if (event - WAIT_OBJECT_0 < numEvents &&
...
*/
HANDLE* IPCStub::GetEventArray (size_t& size, int bucketNumber)
{
    ::WaitForSingleObject (m_hEventListMutex, INFINITE);
    PtrVector* pEventList = (PtrVector*) m_eventBucketList.at (bucketNumber);
    size = pEventList->size ();

    HANDLE* retval = new HANDLE [size];
    int i = 0;
