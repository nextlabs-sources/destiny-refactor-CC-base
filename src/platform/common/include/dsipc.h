...
{
  WCHAR aUserSID[MAX_SID_LENGTH];
  ULONG ulUserSIDLength;
  ULONG ulUserSIDAttribute;
  WCHAR aOwnerSID[MAX_SID_LENGTH];
  ULONG ulOwnerSIDLength;
  WCHAR aGroupSID[MAX_SID_LENGTH];
...
typedef struct tagFILE_INFO
{
  ...
  // ------------------------------------------------------
      ...
      OUT WCHAR	wzFileName[MAX_NAME_LENGTH];
...
typedef struct tagSHAREDNAME_LINKS
{
	...
	WCHAR	wzSharedName[MAX_NAME_LENGTH];
	WCHAR	wzLocalPath[MAX_NAME_LENGTH];
...
typedef struct _IPC_POLICY_REQUEST
{
	...
	WCHAR	szwAction[MAX_ACTION_NAME_SIZE];
