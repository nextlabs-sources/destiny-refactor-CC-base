...
public class CommunicationPolicyDetailsComposite extends Composite {
    ...
    public CommunicationPolicyDetailsComposite(Composite parent, int style, IDPolicy policy) {
        super(parent, style);
        this.policy = policy;
    ...
    private void displayResources() {
        ...
        Label labelToRecipients = new Label(this, SWT.LEFT | SWT.WRAP);
        labelToRecipients.setText("To Recipient(s):");
        labelToRecipients.setFont(FontBundle.ARIAL_9_NORMAL);
        data = new GridData(GridData.FILL_HORIZONTAL);
        labelToRecipients.setLayoutData(data);

        CompositePredicate toSubjects = getToSubjects();
        List<CompositePredicate> toList = filterPredicate(toSubjects);
        if (!toList.isEmpty()) {
            Composite composite = new Composite(this, SWT.NONE);
            data = new GridData(GridData.FILL_HORIZONTAL);
            composite.setLayoutData(data);
            ...
            getUserSubjectLabel(composite, operators1, toList);
        }
    }
    ...
    private CompositePredicate getToSubjects() {
        ITarget target = policy.getTarget();
