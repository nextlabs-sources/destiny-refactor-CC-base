...
#
# DESCRIPTION
#       This script finds the latest build directory (based on build number, not date) with
#       build artifacts. The full path of the build artifact file is saved in a bash script.
#       If no artifact file is found under the sepcified directory then nothing will be written
#
...
print "NextLabs Newest Build Artifact File Locator v0.2\n";
...
#	The name of the artifact file is returned in the $refFilename parameter.

sub getNewestArtifactFile()
{
	my	($path, $refFilename) = @_;
	...
	# Get largest build number with a valid
	my	$maxVal = 0;
	my	$artifactFileName = '';
	my	$fullArtifactFileName = '';
	
	opendir(my $dh, "$path") || die "ERROR: Failed to open directory $path";
	...
	while (my $entry = readdir($dh))
	{
		...
		if ($entry =~ /^(\d+)/)
		{
			if ($1 > $maxVal)
			{
                            # Does this have an artifact file?
                            if (&getProductBuildArtifactFile($path . '/' . $entry, \$artifactFileName)) {
				$maxVal = $1;
				$fullArtifactFileName = $path . '/' . $entry . '/' . $artifactFileName;
                            }
			}		
		}
	}
	...
	{	
		...
		print "Artifact file = $fullArtifactFileName\n";
	}
	...
	${$refFilename} = $fullArtifactFileName; 
	return 1;	
}
...
sub getProductBuildArtifactFile()
{
	...
	opendir(my $fh, "$path") || die "ERROR: Failed to open directory $path";
...
if (!&getNewestArtifactFile($g_startPath, \$file))
{
    exit 1;
}
